...
func _createNotes(session gosn.Session, input map[string]string) (so gosn.SyncOutput, err error) {
	var newNotes gosn.Items
...
func _createTags(session gosn.Session, input []string) (output gosn.SyncOutput, err error) {
	for _, tt := range input {
		newTag := gosn.NewTag()
		newTagContent := gosn.TagContent{
			Title: tt,
		}
		newTagContent.SetUpdateTime(time.Now())
		newTag.Content = newTagContent

		dItems := gosn.Items{&newTag}
		eItems, _ := dItems.Encrypt(session.Mk, session.Ak, true)
		si := gosn.SyncInput{
			Session: session,
			Items:   eItems,
		}
		output, err = gosn.Sync(si)
...
func _getItems(session gosn.Session, itemFilters gosn.ItemFilters) (items gosn.Items, err error) {
	si := gosn.SyncInput{
		Session: session,
	}
...
//func createNote(title, text, uuid string) *gosn.Note {
//	note := gosn.NewNote()
//	if uuid != "" {
//		note.UUID = uuid
//	}
...
func createTag(title, uuid string) *gosn.Tag {
	tag := gosn.NewTag()
	if uuid != "" {
		tag.UUID = uuid
	}
